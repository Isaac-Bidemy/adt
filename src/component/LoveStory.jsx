import React from 'react'
import { Link } from 'react-router-dom';
export default function LoveStory() {
  return (
    
    <div className="max-w-2xl mx-auto p-4 bg-white  justify-center mt-12 rounded-2xl  shadow-md">
      <div> 
      <Link to="/Adetutu"><button
           className="w-full bg-gray-500 text-white py-2 px-4 rounded-md hover:bg-black"
         > ADETUTU</button></Link>
         </div>
      <h1 className="text-3xl font-bold mb-4">My Love Story</h1>
      <p className="text-lg text-gray-600 mb-4">
     
        The very first moment I set my eyes on you, Adetutu, I knew that you were the one for me.
      </p>
      <h2 className="text-2xl font-bold mb-4">You Are My World</h2>
      <p className="text-lg text-gray-600 mb-4">
        You are my sunshine, my guiding star, and my soulmate. You light up my world with your presence, and I am forever grateful to have you by my side.
      </p>
      <h2 className="text-2xl font-bold mb-4">I've Been Praying for You</h2>
      <p className="text-lg text-gray-600 mb-4">
        Ever since you were young, I've been praying for you, Adetutu. I've been asking God to guide you, to protect you, and to bring you joy.
      </p>
      <h2 className="text-2xl font-bold mb-4">Pick Up Lines</h2>
      <ul className="list-disc text-lg text-gray-600 mb-4">
        <li>Are you a magician? Every time I look at you, everyone else disappears.</li>
        <li>Do you have a map? I just keep getting lost in your eyes.</li>
        <li>Excuse me, but I think you dropped something: my jaw.</li>
      </ul>
      <h2 className="text-2xl font-bold mb-4">My Promise to You</h2>
      <p className="text-lg text-gray-600 mb-4">
        Adetutu, I promise to love you, to cherish you, and to make you happy through out our life time.
      </p>
      <h2 className="text-2xl font-bold mb-4">I Love You</h2>
      <p className="text-lg text-gray-600 mb-4">
        Adetutu, I love you more than words can express. You are my everything, my reason for being, and my soulmate. Forever and always, my love.
      </p>
      <h1 className="text-3xl font-bold mb-4">Letter from Mastermind</h1>
      <p className="text-lg text-gray-600 mb-4">
      From the moment I met you, I knew that you were someone special. Your smile lit up the room, your laughter was infectious, and your eyes sparkled with a beauty that I'd never seen before. But it wasn't just your physical beauty that caught my attention - it was the kindness, compassion, and love that radiated from your very being.

As I got to know you better, I realized that you're not just an amazing person - you're the missing piece to my puzzle. You're the one who makes me feel whole, who brings light into my darkness, and who gives me a reason to wake up every morning.

Your touch ignites a fire within me, a flame that burns brighter with every passing day. Your kisses leave me breathless, your hugs make me feel safe, and your love gives me the strength to face anything that life throws my way.

You're the melody that fills my heart with joy, the rhythm that makes me want to dance, and the harmony that brings balance to my life. You're the sunshine that brightens up my day, the stars that light up my night, and the moon that guides me through life's ups and downs.

I love the way you make me laugh, the way you challenge me to be my best self, and the way you support me in everything I do. I love the way you always know how to make me feel better when I'm down, the way you listen to me when I need someone to talk to, and the way you always encourage me to pursue my dreams.

I love you for who you are, and for who you help me to be. I love you for your strengths and your weaknesses, for your passions and your fears. I love you for your beauty, inside and out, and for the way you make me feel like I'm home.

Ade Tutu, I don't know what the future holds, but I do know that I want to spend it with you. I want to wake up every morning with you by my side, to explore the world with you, and to build a life together that's filled with love, laughter, and adventure.

I love you more than words can express, more than any poem or song or work of art can capture. I love you with all my heart, with all my soul, and with every fiber of my being.

Yours always,
[ISAAC-BIDEMY]

</p>
      
         
    </div>
  );
}